...
import ch.joelniklaus.ecogame.controller.service.SearchService;
...
import ch.joelniklaus.ecogame.model.dao.SearchDao;
...
public class SearchController {
	 ...
	 *
	 * @param searchId
	 *            id of search-template to be used
	 *
	...
	/**
	 * Assembles a model providing search functionality (specifying search criteria, displaying only
	 * ads matching that criteria).
	 *
	 * @param searchForm
	...
	public ModelAndView search(@Valid SearchForm searchForm, BindingResult result,
		...
		if (searchId != null)
			searchAttributes = new Search(new Long(0), new Long(0), new Long(3000), new Long(0),
					new Long(300), "", "", "");
		else {
			...
			searchAttributes = new Search(new Long(0), priceMin, priceMax, roomSizeMin,
					roomSizeMax, city, earliestDateIn, latestDateOut);
		}
	...
	public ModelAndView saveSearch(@Valid SearchForm searchForm, BindingResult result,
			RedirectAttributes redirectAttributes,
			@RequestParam(value = "searchId", required = false) String searchId) {
